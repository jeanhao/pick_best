...
STONE_MAX_SIZE = 10
STONE_INDEX = list(range(STONE_MAX_SIZE))
STONE_POS_X = [OBJECT_WIDTH * i for i in range(STONE_MAX_SIZE)]
# STONE_SIZE = [7, 8, 9, 10]
STONE_SIZE = [10]
# STONE_SIZE = 8
STONE_VALUE = [-10, -5, -3, -2, -1, 1, 2, 3, 5, 10]
STONE_VALUE_COLOR = {STONE_VALUE[i] : (50 + 20 * i,) * 3 for i in range(len(STONE_VALUE))}
STONE_UPDATE_DISTANCE = 150  # 两层石头间的距离 + 50
STONE_SPEED = 5
...
class GameObject(object):
    ...
    def __init__(self):
        ...
        self.reward_font = pygame.font.SysFont("arial", 15)
        self.com_init()
    ...
    def init(self):
        self.play = False
        self.player_pos = 1
        self.stones_pos = deque()
        self.stones_val = deque()
    ...
    def update_screen(self):
        ...
        # 画石头
        for i in range(len(self.stones_pos)):
            stone_i = self.stones_pos[i]
            for j in range(len(stone_i)):
                stone_pos = stone_i[j]
#         for stones_row in self.stones_pos:
#             for stone_pos in stones_row:

#                 self.screen.blit(self.stone_val_images[self.stones_val[i][j]], stone_pos)
                rect = Rect(stone_pos, OBJECT_SIZE)
                pygame.draw.rect(self.screen, STONE_VALUE_COLOR[self.stones_val[i][j]], rect)
#                 reward_surface = self.reward_font.render(str(self.stones_val[i][j]), True, BLACK)
#                 self.screen.blit(reward_surface, ((stone_pos[0] + OBJECT_SIZE[0] - reward_surface.get_width()) / 2,
#                                      (stone_pos[1] + OBJECT_SIZE[1] - reward_surface.get_height()) / 2))
        pygame.display.update()

    def gen_stones(self):  # 随机生成位置
        stone_size = random.choice(STONE_SIZE)
        stones_pos = [[pos, STONE_INIT_POS_Y] for pos in random.sample(STONE_POS_X, stone_size)]
        stones_val = random.sample(STONE_VALUE, stone_size)
        self.stones_pos.append(stones_pos)
        self.stones_val.append(stones_val)
#         return [[pos, STONE_INIT_POS_Y] for pos in random.sample(STONE_POS_X, STONE_SIZE)]

    def update_stones(self):
        # 看是否需要产生新的石头
        if not self.stones_pos:  # 如果没有石头
            self.gen_stones()
        else:
            # 获取最后一组stone的第一个的其y坐标
            last_stone_y = self.stones_pos[-1][0][1]
            if last_stone_y > STONE_UPDATE_DISTANCE:
                self.gen_stones()

        # 更新所有石头距离
        for row in range(len(self.stones_pos)):
            for col in range(len(self.stones_pos[row])):
                self.stones_pos[row][col][1] += STONE_SPEED  # 第col块石头y坐标

        # 检查第一组石头是否和player发生碰撞
        reward = 0
#         print(self.stones_pos)
        if len(self.stones_pos) > 0:
            stone_y = self.stones_pos[0][0][1]
            if SCREEN_HEIGHT >= stone_y > SCREEN_HEIGHT - OBJECT_HEIGHT * 2 :  # 检查两个石头是否和用户发生碰撞
                for col in range(len(self.stones_pos[0])):
                    if self.stones_pos[0][col][0] / OBJECT_WIDTH == self.player_pos:
                        reward = self.stones_val[0].pop(col)
                        self.stones_pos[0].pop(col)
                        break
            elif stone_y > SCREEN_HEIGHT:  # 已经过了，给用户加分
    #             reward = 1
                self.stones_pos.popleft()  # 清空两块石头
                self.stones_val.popleft()
        return reward
...
#             self.init()
        ...
        # 更新速度
        self.update_score_speed()
        image_data = pygame.surfarray.array3d(pygame.display.get_surface())[:MENU_LINE_X]
        self.update_screen()
